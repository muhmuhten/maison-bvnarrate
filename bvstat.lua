...
local function dothething (data, label)
	...
	--print(label, "P1="..strutf8(data, 237, 13))
	--print(label, "Mode="..data:byte(1), "Style="..data:byte(2), "Battle #"..unpack("<I2", data, 401))
	--print(label, ("ts %d-%02d-%02d %02d:%02d:%02d"):format(unpack("<I2BBBBB", data, -16)))

	local party1 = decrypt(unpack("c260", data, 3609))
	for j=1,6 do
		local pkm = decrypt(unpack("c260", data, 3609 + 1568 + 260*(j-1)))
		--[[
		if data:byte(1) ~= 2 or (unpack("<I2", data, 401) <= 40 and os.getenv("MAISON_OMIT_SUB40")) then
			break
		end
		--]]
		...
		print(
			--strutf8(data, 237, 26) .. "／" ..
			strutf8(party1, 65, 26),
			("%d-%02d-%02d %02d:%02d:%02d"):format(unpack("<I2BBBBB", data, -16)),
			label:sub(-3),
			data:byte(1) .. data:byte(2), -- Mode-Style
			unpack("<I2", data, 401), -- Battle #
			j,
			--[[
			-- Species-Nature-Item-Move4-Ability
			table.concat({
					unpack("<I2", pkm, 9),
					pkm:byte(29)//5+1 .. "," .. pkm:byte(29)%5+1,
					unpack("<I2", pkm, 11),
					unpack("<I2", pkm, 97),
				nil}, "-") .. "/" .. pkm:byte(21),
			--]]
			...
			--strutf8(pkm, 65, 26),
			jtrainers[data:byte(11485)+1],
			sets[data:byte(11485)+1],
			table.unpack{os.getenv("WANT_FULL_PATH") and label or nil})
			
	end

	--[[
	for j=1,3 do
		local pkm = decrypt(unpack("c260", data, 3609 + (6*260+8)*1 + 260*(j-1)))
		local nature = pkm:byte(29)
		print(label, "Enemy #"..j, strutf8(pkm, 65, 26),
			"Species="..unpack("<I2", pkm, 9),
			"Item="..unpack("<I2", pkm, 11),
			"Nature="..pkm:byte(29).."/+"..nature//5+1 ..",-"..nature%5+1,
			"Move4="..unpack("<I2", pkm, 97))
	end
	--]]
	--io.write(pkm)
	--print(label, ("%q"):format(pkm))
end
